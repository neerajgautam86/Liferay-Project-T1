...
public class CustomArchiveFileEntryAction extends BaseStrutsPortletAction {
	...
	public void processAction(StrutsPortletAction originalStrutsPortletAction, PortletConfig portletConfig, ActionRequest actionRequest,
		...
		LOGGER.info("CustomArchiveFileEntryAction class is called for processAction.");
		...
		if (cmd.equals("restore_archive")) {
			...
			innerForLoop: for (DDMStructure struct : structures) {
				DLFileEntryMetadata dlFileEntryMetadata = DLFileEntryMetadataLocalServiceUtil.getFileEntryMetadata(struct.getStructureId(),
		...
		/*
		 * else if (cmd.equals("delete")) { for (int i = 0; i <
		 * fileEntryIds.length; i++) { long deleteFileEntryId = fileEntryIds[i];
